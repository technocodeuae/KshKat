...
class AllBestSellerPage extends StatefulWidget {
  // final token;
  final UserSignInDashboardResult? userInfo;

  const AllBestSellerPage({this.userInfo});

  @override
  _AllBestSellerPageState createState() => _AllBestSellerPageState();
}

class _AllBestSellerPageState extends State<AllBestSellerPage>
    with SingleTickerProviderStateMixin, AutomaticKeepAliveClientMixin {
  CategoryController categoryController = Get.find();

  var _cancelToken = CancelToken();
  int limit = 25, page = 1, startAt = 0, endAt = 0, preparePage = 1;
  late List<LatestProductData> bestList = <LatestProductData>[];
  ...
  Widget _buildBody({required double width, required double height}) {
    return GetBuilder<CategoryController>(
        ...
        builder: (state) {
          ...
          if (state.bestSellerSuccess != null &&
              _isTopProduct == true &&
              (GeneralState.iSuccess) &&
              (state.bestSellerSuccess ==
                  categoryController.bestSellerSuccess)) {
            if (page == 1) {
              bestList = state.bestSellerSuccess!.result.data!;
            } else {
              bestList.addAll(state.bestSellerSuccess!.result.data!);
            }
