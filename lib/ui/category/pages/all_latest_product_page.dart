...
class AllLatestProductPage extends StatefulWidget {
  // final token;
  final UserSignInDashboardResult? userInfo;

  const AllLatestProductPage({/*required this.token,*/ this.userInfo});

  @override
  _AllLatestProductPageState createState() => _AllLatestProductPageState();
}

class _AllLatestProductPageState extends State<AllLatestProductPage>
  ...
  var token, address, time, timeId, arg;
  List<Products> cart = [];
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: appBar,
      body: SafeArea(child: _buildBody(height: heightC, width: widthC)),
    );
  }
  ...
  Widget _buildBody({required double width, required double height}) {
    return GetBuilder<CategoryController>(
        ...
        builder: (state) {
          ...
          if (state.latestProductSuccess != null &&
              _isLatestProduct &&
              (GeneralState.iSuccess) &&
              (state.latestProductSuccess ==
                  categoryController.latestProductSuccess)) {
            if (page == 1) {
              latestList = state.latestProductSuccess!.result.data!;
            } else {
              latestList.addAll(state.latestProductSuccess!.result.data!);
            }
